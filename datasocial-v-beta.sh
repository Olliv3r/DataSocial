...
# Finaliza todos os processos relacionados aos túneis e servidor local (php, ngrok, ssh e /scloudflared).
killAllProcess() {
	local processes=("php" "ngrok" "ssh" "cloudflared")
...
listenServerLocalhost() {
	...
	php -S "$host:$port" -t "$www" > logs/php.log 2>&1 &
...
listenServerPublic() {
	...
	case "$tunnelSelected" in
		...
		ssh)
			...
			ssh -i $HOME/.ssh/id_rsa -R 80:localhost:$port ssh.localhost.run > logs/ssh.log 2>&1 &
			
			link=$(waitTunnelLink "logs/ssh.log" "https://[a-z0-9]+\.lhr\.life" | head -n 1)
			;;
			
		cloudflared)
			cloudflared tunnel --url http://$host:$port > logs/cloudflared.log 2>&1 &
